...
export function AdminSidebar() {
  ...
  // Estado para grupos colapsados - por padrão todos começam expandidos
  const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>({});
  
  const collapsed = state === "collapsed";
  const currentPath = location.pathname;
  
  // Função para alternar colapso de um grupo
  ...
  // Verificar se o grupo contém a rota ativa (para manter aberto automaticamente)
  const groupContainsActiveRoute = useCallback((items: Array<{ url: string }>) => {
  ...
  const getNavigationItems = () => {
    ...
    if (userRole === 'attendant') {
      ...
      const allMenuItems: Array<{ title: string; url: string; icon: any; group: string; permissionKey: PermissionKey | null }> = [
        ...
        { title: 'Chat WhatsApp', url: '/dashboard/whatsapp/chat', icon: MessageSquare, group: 'WhatsApp', permissionKey: 'whatsapp_chat' },
    ...
    if (profile?.user_type === 'master_admin') {
      return [
        ...
        { title: 'Fila Follow-up', url: '/dashboard/follow-up-queue', icon: Phone, group: 'Vendedores' },
      ...
      if (hasModule('booking')) {
        menuItems.push(
          ...
          { title: 'Pagamentos', url: '/dashboard/booking/payments', icon: Banknote, group: 'Agendamento' },
        ...
        {Object.entries(groupedItems).map(([groupName, items]) => {
          ...
          return (
            <SidebarGroup key={groupName}>
              {(!collapsed || isMobile) ? (
                <Collapsible open={isOpen} onOpenChange={() => toggleGroupCollapse(groupName)}>
                  <CollapsibleTrigger asChild>
                    <button 
                      className="w-full flex items-center justify-between text-xs font-semibold text-muted-foreground px-4 py-2 hover:text-foreground hover:bg-muted/50 rounded-md transition-colors"
                    >
                      <span>{groupName}</span>
                      {isOpen ? (
                ...
                // Quando colapsado, mostrar apenas os ícones sem agrupamento
                <SidebarGroupContent>
                  <SidebarMenu>
                    {items.map((item) => (
                      <SidebarMenuItem key={item.title}>

...
export function WebhookConfigCard({ 
  ...
  const webhookEndpoint = `https://noshwvwpjtnvndokbfjx.supabase.co/functions/v1/efi-pix-webhook`;

  const handleConfigureWebhook = async () => {
    setConfiguring(true);
    try {
      const { data, error } = await supabase.functions.invoke('efi-configure-webhook');
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Receba notificações automáticas quando um pagamento PIX for confirmado
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
          ...
          <ul className="text-sm text-muted-foreground space-y-1">
            ...
            <li className="flex items-center gap-2">
              <CheckCircle2 className="h-3 w-3 text-green-500" />
              Funciona mesmo se usuário fechar a página
            </li>
            <li className="flex items-center gap-2">
              <CheckCircle2 className="h-3 w-3 text-green-500" />
        ...
        {webhookUrl && (
          <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
            <p className="text-sm font-medium text-green-700 mb-1">✅ Webhook ativo</p>
        ...
        {!isConfigured && (
          <p className="text-xs text-muted-foreground text-center">
            ⚠️ Salve e teste a conexão primeiro antes de configurar o webhook
          </p>
        )}
        ...
        <div className="pt-3 border-t">
          <p className="text-xs text-muted-foreground">
            <strong>Nota:</strong> O webhook será configurado automaticamente via API. 
            Não é necessário configurar manualmente no painel EFI.
          </p>
        </div>
      </CardContent>
    </Card>

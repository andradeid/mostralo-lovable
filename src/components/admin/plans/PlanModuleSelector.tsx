...
export function PlanModuleSelector({ planId, selectedModuleIds, onModulesChange }: PlanModuleSelectorProps) {
  ...
  const advancedModules = modules.filter(m => 
    ['reports', 'printing', 'scheduled_orders', 'booking', 'delivery_drivers', 'promotions', 'password_call', 'kds', 'self_service_table', 'whatsapp_connection'].includes(m.key || '')
  );
  const premiumModules = modules.filter(m => 
    ['marketing', 'integrations', 'custom_scripts', 'marketing_material', 'financial_management', 'ifood_integration', 'whatsapp_recovery', 'banners', 'attendants', 'digital_signage', 'sentinela', 'self_service_totem', 'upsell', 'whatsapp_chat', 'whatsapp_ai', 'commercial_analysis'].includes(m.key || '')
  );

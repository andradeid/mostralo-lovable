...
import { CreditCard, Banknote, QrCode, Wallet, Loader2, Printer, X } from 'lucide-react';
      ...
      <div className="bg-muted rounded-lg p-4 space-y-3">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor="service-fee" className={`cursor-pointer ${isMobile ? 'text-base' : 'text-sm'}`}>
              Taxa {serviceFeePercentage}%
            </Label>
          </div>
          {includeServiceFee && (
      ...
      {selectedMethod === 'dinheiro' && (
        <div className="space-y-3 p-4 bg-muted/50 rounded-lg">
          <div className="space-y-2">
            <Label htmlFor="received-close" className={isMobile ? 'text-base' : ''}>Valor Recebido</Label>
            <Input
              id="received-close"
              type="number"
              min={0}
      ...
      <Drawer open={open} onOpenChange={handleClose}>
        <DrawerContent className="max-h-[90vh]">
          <DrawerHeader className="text-left">
            <DrawerTitle className="text-xl">Fechar Comanda</DrawerTitle>
            <DrawerClose asChild>

import { useState } from 'react';
import { Share2, Copy, Mail, Check } from 'lucide-react';
import { Button } from '@/components/ui/button';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from '@/components/ui/popover';
import { cn } from '@/lib/utils';
import type { CardTheme } from '@/types/digitalCard';
import { toast } from '@/hooks/use-toast';

interface ShareCardButtonProps {
  cardUrl: string;
  cardName: string;
  theme?: CardTheme;
  accentColor?: string;
  variant?: 'card' | 'admin';
  onShare?: (method: string) => void;
}

const themeStyles: Record<CardTheme, { card: string; text: string }> = {
  dark: { card: 'bg-zinc-800/50 border-zinc-700', text: 'text-white' },
  light: { card: 'bg-white/80 border-gray-200', text: 'text-gray-900' },
  orange: { card: 'bg-orange-700/30 border-orange-400/30', text: 'text-white' },
  gradient: { card: 'bg-violet-800/30 border-violet-500/30', text: 'text-white' },
};

export function ShareCardButton({ 
  cardUrl, 
  cardName, 
  theme = 'light', 
  accentColor = '#f97316',
  variant = 'card',
  onShare 
}: ShareCardButtonProps) {
  const [copied, setCopied] = useState(false);
  const [open, setOpen] = useState(false);
  const styles = variant === 'admin' ? null : themeStyles[theme];

  const shareText = `Confira o cartão digital de ${cardName}`;
  const canUseNativeShare = typeof navigator !== 'undefined' && !!navigator.share;

  const handleNativeShare = async () => {
    try {
      await navigator.share({
        title: `Cartão Digital - ${cardName}`,
        text: shareText,
        url: cardUrl,
      });
      onShare?.('native_share');
    } catch (error) {
      // User cancelled or error occurred
      if ((error as Error).name !== 'AbortError') {
        console.error('Error sharing:', error);
      }
    }
  };

  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText(cardUrl);
      setCopied(true);
      onShare?.('copy_link');
      toast({
        title: 'Link copiado!',
        description: 'O link foi copiado para a área de transferência.',
      });
      setTimeout(() => setCopied(false), 2000);
      setOpen(false);
    } catch (error) {
      console.error('Error copying:', error);
    }
  };

  const handleWhatsAppShare = () => {
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${shareText}: ${cardUrl}`)}`;
    window.open(whatsappUrl, '_blank');
    onShare?.('whatsapp');
    setOpen(false);
  };

  const handleEmailShare = () => {
    const subject = encodeURIComponent(`Cartão Digital - ${cardName}`);
    const body = encodeURIComponent(`${shareText}\n\n${cardUrl}`);
    window.open(`mailto:?subject=${subject}&body=${body}`, '_blank');
    onShare?.('email');
    setOpen(false);
  };

  const handleTwitterShare = () => {
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(cardUrl)}`;
    window.open(twitterUrl, '_blank');
    onShare?.('twitter');
    setOpen(false);
  };

  // On mobile, use native share
  if (canUseNativeShare) {
    return (
      <Button
        variant="ghost"
        size="icon"
        className={cn(
          'rounded-full',
          variant === 'admin' 
            ? 'h-8 w-8 hover:bg-muted text-muted-foreground hover:text-foreground' 
            : cn(styles?.card, styles?.text)
        )}
        onClick={handleNativeShare}
        aria-label="Compartilhar"
      >
        <Share2 
          className={variant === 'admin' ? 'w-4 h-4' : 'w-5 h-5'} 
          style={variant === 'card' ? { color: accentColor } : undefined} 
        />
      </Button>
    );
  }

  // On desktop, show popover with options
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className={cn(
            'rounded-full',
            variant === 'admin' 
              ? 'h-8 w-8 hover:bg-muted text-muted-foreground hover:text-foreground' 
              : cn(styles?.card, styles?.text)
          )}
          aria-label="Compartilhar"
        >
          <Share2 
            className={variant === 'admin' ? 'w-4 h-4' : 'w-5 h-5'} 
            style={variant === 'card' ? { color: accentColor } : undefined} 
          />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-56 p-2" align="end">
        <div className="space-y-1">
          <p className="text-sm font-medium px-2 py-1.5 text-muted-foreground">
            Compartilhar
          </p>
          
          <Button
            variant="ghost"
            className="w-full justify-start gap-2 h-10"
            onClick={handleWhatsAppShare}
          >
            <svg 
              viewBox="0 0 24 24" 
              className="w-4 h-4 text-green-500 fill-current"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
            </svg>
            WhatsApp
          </Button>
          
          <Button
            variant="ghost"
            className="w-full justify-start gap-2 h-10"
            onClick={handleCopyLink}
          >
            {copied ? (
              <Check className="w-4 h-4 text-green-500" />
            ) : (
              <Copy className="w-4 h-4" />
            )}
            {copied ? 'Copiado!' : 'Copiar Link'}
          </Button>
          
          <Button
            variant="ghost"
            className="w-full justify-start gap-2 h-10"
            onClick={handleEmailShare}
          >
            <Mail className="w-4 h-4" />
            Email
          </Button>
          
          <Button
            variant="ghost"
            className="w-full justify-start gap-2 h-10"
            onClick={handleTwitterShare}
          >
            <svg 
              viewBox="0 0 24 24" 
              className="w-4 h-4 fill-current"
            >
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
            </svg>
            Twitter / X
          </Button>
        </div>
      </PopoverContent>
    </Popover>
  );
}

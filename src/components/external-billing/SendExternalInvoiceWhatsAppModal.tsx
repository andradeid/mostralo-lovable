...
interface SendExternalInvoiceWhatsAppModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  invoice: ExternalInvoice | null;
}

export function SendExternalInvoiceWhatsAppModal({
  open,
  onOpenChange,
  invoice,
}: SendExternalInvoiceWhatsAppModalProps) {
...
Qualquer dúvida, estamos à disposição!`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('send-external-invoice-whatsapp', {
        body: {
      ...
      if (data?.success) {
        ...
        throw new Error(data?.error || 'Erro ao enviar fatura');
      }
    } catch (error) {
      console.error('Erro ao enviar fatura:', error);
      toast.error(error instanceof Error ? error.message : 'Erro ao enviar fatura por WhatsApp');
    } finally {
      setIsSending(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Envie a fatura #{invoice.invoice_number || invoice.id.slice(0, 8).toUpperCase()} para o cliente
          </DialogDescription>
        </DialogHeader>

...
export function ReceiptActions({ receiptRef, invoice }: ReceiptActionsProps) {
  ...
  const receiptUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}/receipt/${invoice.id}` 
    : '';
  ...
  const handlePrint = useCallback(() => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Recibo - ${invoice.stores?.name || 'Mostralo'}</title>
          <meta charset="utf-8">
          <style>
  ...
  }, [receiptRef, invoice.stores?.name]);
...
${receiptUrl}`;
  ...
  const handleShareEmail = useCallback(() => {
    const subject = `Recibo de Pagamento - ${invoice.stores?.name || 'Mostralo'}`;
    const body = `Olá!
  ...
  const handleNativeShare = useCallback(async () => {
    ...
    try {
      await navigator.share({
        title: `Recibo de Pagamento - ${invoice.stores?.name || 'Mostralo'}`,

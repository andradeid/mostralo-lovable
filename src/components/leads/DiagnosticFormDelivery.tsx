...
import type { 
  DeliveryDiagnosticAnswers, 
  ContactData, 
  BusinessNiche 
} from '@/lib/diagnosticScoringDelivery';
import { 
  NICHE_CONFIG, 
  NICHE_QUESTIONS, 
  DEPENDENCIA_OPTIONS, 
...
interface DiagnosticFormDeliveryProps {
  onComplete: (answers: DeliveryDiagnosticAnswers, contact: ContactData) => void;
}

interface Question {
  id: 'nicho' | 'dependencia' | 'volume' | 'desafio';
  title: string;
  question: string;
...
export function DiagnosticFormDelivery({ onComplete }: DiagnosticFormDeliveryProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [answers, setAnswers] = useState<Partial<DeliveryDiagnosticAnswers>>({});
  ...
  const getQuestions = useCallback((): Question[] => {
    const selectedNiche = answers.nicho as BusinessNiche | undefined;
    const nicheQuestions = selectedNiche ? NICHE_QUESTIONS[selectedNiche] : NICHE_QUESTIONS.outro;
    ...
    return [
      {
        ...
        options: Object.entries(NICHE_CONFIG).map(([value, config]) => ({
          value,
          label: `${config.icon} ${config.label}`
        }))
      },
      {
        id: 'dependencia' as const,
        title: 'Dependência de Apps',
        question: nicheQuestions.dependencia,
        options: DEPENDENCIA_OPTIONS.map(opt => ({ value: opt.value, label: opt.label }))
      },
      {
        id: 'volume' as const,
        title: 'Volume de Pedidos',
        question: nicheQuestions.volume,
        options: VOLUME_OPTIONS.map(opt => ({ value: opt.value, label: opt.label }))
      },
      {
        id: 'desafio' as const,
        title: 'Maior Desafio',
        question: nicheQuestions.desafio,
  ...
  const handleSubmit = async () => {
    if (contact.name && contact.phone && contact.company && !isSubmitting && isWhatsappValidated) {
      ...
      setTimeout(() => {
        onComplete(answers as DeliveryDiagnosticAnswers, {
          ...contact,

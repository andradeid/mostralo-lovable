...
interface CompanyInfoEditorProps {
  mode?: 'full' | 'compact' | 'readonly';
  title?: string;
  description?: string;
  showPlaceholderInfo?: boolean;
  onSaveSuccess?: () => void;
  onDataLoaded?: (data: ContractorInfo) => void;
  className?: string;
}

const defaultContractorInfo: ContractorInfo = {
  company_name: '',
  cnpj: '',
  address: '',
  city: '',
...
export const CompanyInfoEditor = ({
  ...
  description = 'Informações oficiais da empresa para contratos',
  showPlaceholderInfo = true,
  onSaveSuccess,
  onDataLoaded,
  className,
}: CompanyInfoEditorProps) => {
  const [contractorInfo, setContractorInfo] = useState<ContractorInfo>(defaultContractorInfo);
  ...
  const fetchContractorInfo = async () => {
    try {
      ...
      if (!error && data) {
        const info = data.value as unknown as ContractorInfo;
  ...
  // Modo readonly - apenas exibe os dados
  if (mode === 'readonly') {
    return (
      <div className={`space-y-3 ${className}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="font-medium">{contractorInfo.cnpj || '-'}</p>
          </div>
          <div>
            <span className="text-muted-foreground">Razão Social:</span>
            <p className="font-medium">{contractorInfo.company_name || '-'}</p>
          </div>
        </div>
  ...
  // Modo compact - formulário reduzido
  if (mode === 'compact') {
    return (

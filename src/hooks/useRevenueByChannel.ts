...
const CHANNEL_CONFIG: Record<string, { color: string; icon: string; label: string }> = {
  ...
  'Vendas iFood': { color: '#ea1d2c', icon: 'Smartphone', label: 'iFood' },
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RevenueByChannelData> => {
      ...
      const { data: categories, error: catError } = await supabase
        ...
        .select('id, name')
        .eq('type', 'income')
        .in('name', ['Vendas', 'Vendas iFood', 'Vendas Totem', 'Vendas PDV', 'Vendas Mesa', 'Agendamentos']);
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        (transactions || []).forEach(tx => {
          ...
          if (txMonth === monthKey) {
            ...
            else if (categoryName === 'Vendas iFood') monthData.ifood += amount;

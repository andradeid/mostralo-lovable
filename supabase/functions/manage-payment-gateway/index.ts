...
serve(async (req) => {
  ...
  try {
    ...
    // ==================== GET: Retorna metadados (sem tokens) ====================
    if (method === "GET") {
      ...
      const { data: isAdmin } = await supabaseAdmin.rpc("is_store_admin_of", {
        _store_id: storeId,
      });

      // Buscar gateway sem expor tokens completos
    ...
    if (method === "POST") {
      ...
      const { store_id, access_token, public_key, environment } = body;
      ...
      // Verificar se o usuário é admin da loja
      ...
      // Verificar permissão
      const { data: hasRole } = await supabaseAdmin.rpc("is_store_admin_of", {
        _store_id: store_id,
      });
      ...
      try {
        ...
        console.error("Erro validação MP:", e);
      }

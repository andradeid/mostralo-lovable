...
function getSmartGreetingTemplate(
  ...
  if (!isOpen) {
    ...
    switch (classification.type) {
      case 'vip':
        return `${greeting}, {primeiro_nome}! 🌟 Nosso cliente especial!\n\n${closedMessage}\n\nMas enquanto isso, que tal dar uma olhada nos nossos produtos?${menuLink}`;
      case 'frequent':
      case 'returning':
        return `${greeting}, {primeiro_nome}! 😊\n\n${closedMessage}\n\nDá uma olhada nos produtos para quando abrirmos!${menuLink}`;
      case 'missed':
        return `${greeting}, {primeiro_nome}! Que bom ter você de volta! 💕\n\n${closedMessage}\n\nMas já confira nossos produtos!${menuLink}`;
      default:
